...
import type { Bike as BikeType, BikeType as BikeTypeEnum, BikeStatus, BikeFormData, BikeFormErrors } from '../types';
...
const BIKE_STATUSES: BikeStatusInfo[] = [
  ...
  { value: 'maintenance', label: 'Održavanje', color: '#f59e0b', icon: AlertCircle },
  { value: 'disabled', label: 'Isključen', color: '#ef4444', icon: X },
];

function StatusBadge({ status }: { status: BikeStatus }): JSX.Element {
  const statusInfo = BIKE_STATUSES.find(s => s.value === status) || BIKE_STATUSES[0];
...
function BikeModal({ bike, isOpen, onClose, onSave }: BikeModalProps): JSX.Element | null {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              type="text"
              value={form.label}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setForm({ ...form, label: e.target.value })}
          ...
          <div>
            ...
            <select
              value={form.type}
              onChange={(e: ChangeEvent<HTMLSelectElement>) => setForm({ ...form, type: e.target.value as BikeTypeEnum })}
              className="select"
            >
              {BIKE_TYPES.map(type => (
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) => setForm({ ...form, pricePerHour: e.target.value })}
          ...
          <div>
            <label className="label flex items-center gap-2">
              <Map size={16} />
              Lokacija bicikla * <span className="text-slate-500 font-normal">(kliknite na mapu)</span>
            </label>
            
            {/* Map Picker */}
            <div className="rounded-xl overflow-hidden border border-slate-700 mb-3" style={{ height: '200px' }}>
              <MapContainer
                center={[
          ...
          <div>
            ...
            <select
              value={form.status}
              onChange={(e: ChangeEvent<HTMLSelectElement>) => setForm({ ...form, status: e.target.value as BikeStatus })}
              className="select"
            >
...
function StatusChangeModal({ bike, isOpen, onClose, onSave }: StatusChangeModalProps): JSX.Element | null {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-white">Promeni status bicikla</h2>
          <button onClick={onClose} className="p-2 hover:bg-slate-700 rounded-lg transition">
            <X size={20} className="text-slate-400" />
          </button>
        </div>
          ...
          {BIKE_STATUSES.map(s => {
            ...
            return (
              ...
              >
                ...
                <span className={status === s.value ? 'text-white' : 'text-slate-300'}>
                  {s.label}
                </span>
                {status === s.value && (
...
function DeleteConfirmModal({ bike, isOpen, onClose, onConfirm }: DeleteConfirmModalProps): JSX.Element | null {
  const [loading, setLoading] = useState<boolean>(false);
  const { showSuccess, showError } = useToast();
    ...
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <p className="text-slate-400 mb-6">
          Da li ste sigurni da želite da obrišete bicikl <span className="text-white font-medium">{bike.label}</span>?
...
function Bikes(): JSX.Element {
  ...
  const filteredBikes = bikes.filter(bike => {
    const matchesSearch = bike.label.toLowerCase().includes(search.toLowerCase()) ||
      ...
      {loading ? (
        ...
        >
          ...
          <h3 className="text-lg font-medium text-white mb-2">
            {bikes.length === 0 ? 'Nema bicikala u sistemu' : 'Nema rezultata pretrage'}
          </h3>
          <p className="text-slate-400 mb-6">
            {bikes.length === 0 
            ...
            <tbody>
              {filteredBikes.map((bike, index) => (
                ...
                >
                  ...
                  <td><TypeBadge type={bike.type} /></td>
                  <td className="text-white font-medium">{bike.pricePerHour} RSD</td>
                  <td>
                    <div className="flex items-center gap-2 text-slate-400">
                      <MapPin size={14} />
                      <span className="text-sm">{bike.lat.toFixed(4)}, {bike.lng.toFixed(4)}</span>
                    </div>
                  </td>
                  <td>
